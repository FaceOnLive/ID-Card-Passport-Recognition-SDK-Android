...
class BottomSheetScrimView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    private var thumbnailRightBitmap: Bitmap? = null
    private var thumbnailRightRect: RectF? = null
    ...
    fun updateWithThumbnailTranslate(
        ...
        thumbnailRightBitmap: Bitmap,
        collapsedStateHeight: Int,
        slideOffset: Float,
        bottomSheet: View
    ) {
        this.thumbnailBitmap = thumbnailBitmap
        this.thumbnailRightBitmap = thumbnailRightBitmap
        ...
        if(thumbnailRightBitmap != null) {
            thumbnailRightRect = RectF().apply {
                val thumbnailRightWidth =
                    thumbnailRightBitmap.width.toFloat() / thumbnailRightBitmap.height.toFloat() * thumbnailHeight.toFloat()
                right = bottomSheet.right -  thumbnailMargin.toFloat()
                top = height.toFloat() - currentSheetHeight - thumbnailMargin.toFloat() - thumbnailHeight.toFloat()
                left = right - thumbnailRightWidth
                bottom = top + thumbnailHeight
            }
        } else {
            thumbnailRightRect = null
        }

        Log.e("ddd", "updateWithThumbnailTranslate " + thumbnailRect)
        invalidate()
    }
    ...
    fun updateWithThumbnailTranslateAndScale(
        ...
        if(thumbnailRightBitmap != null) {
            thumbnailRightRect = RectF().apply {
                var ratio = thumbnailRightBitmap.width / thumbnailRightBitmap.height.toFloat()
                var rightWidth = thumbnailRect!!.height() * ratio

                left = width.toFloat() - thumbnailRect!!.left - rightWidth
                top = thumbnailRect!!.top
                right = width.toFloat() - thumbnailRect!!.left
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (downPercentInCollapsed < DOWN_PERCENT_TO_HIDE_THUMBNAIL) {
            ...
            if(thumbnailRightBitmap != null && thumbnailRightRect != null) {
                canvas.drawBitmap(thumbnailRightBitmap!!, null, thumbnailRightRect!!, thumbnailPaint)
            }
            ...
            if(thumbnailRightRect != null) {
                canvas.drawRoundRect(thumbnailRightRect!!, 6.toFloat(), 6.toFloat(), boxPaint)
            }
        }
    }
